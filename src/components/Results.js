...
export default function Results(props) {
  ...
  // defense bonuses
  if (props.selectedUnitDefenderData.defence_Bonus && !props.selectedUnitDefenderData.cityWall){
    props.selectedUnitDefenderData.default_Bonus = props.selectedUnitDefenderData.default_Bonus * 1.5;
  }
  else if(props.selectedUnitDefenderData.isCityWall){
    props.selectedUnitDefenderData.default_Bonus = props.selectedUnitDefenderData.default_Bonus * 4;
  }
...
<div>
  ...
  <div className="card-body items-center text-center">
    ...
    <h2 className="card-title"> <img className="small-image-size" src="/misc-images/BigSwords.png" />Fight Results <img className="small-image-size" src="/misc-images/BigSwords.png" /></h2>
  ...
  <div className="grid grid-cols-9 gap-1 items-center">
        ...
        <div className="gap-0 justify-self-end damageTaken">(-{defenceResult})</div>
        <div><img src={imageAttacker} width={50} height={50} className="rounded-xl"/> </div>
        <div>Attack:{props.selectedUnitAttackerData.attack} <span className={`hide-text ${props.selectedUnitAttackerData.isBoosted ? 'show-text' : ''}`}>({boostedDamageToUse})</span> </div>
        <div>Defense: {props.selectedUnitAttackerData.defence}</div>
        <div>Original HP {props.selectedUnitAttackerData.inputHealth} (max:{attackerMaxHPToUse}) <span style={{color: "green"}}>{props.selectedUnitAttackerData.isVeteran ?  ` +${veteranBuff}` : ''} </span></div>
          <div>Status:
            <span className={`show-text ${offenceStatus.Title === "Defeated" ? 'show-text-red' : ''}`}>  {offenceStatus.Title}</span>
          </div>
        <div className={` small-image-size ${offenceStatus.Survived ? 'hide-image' : ''}`}>
            <img  src="/misc-images/Vengir_Skull.png" />
          </div>
        <div>Defense</div>
        <div>({defence_status_HP})</div>
        <div className="gap-0 justify-self-end damageTaken">(-{attackResult})</div>
        <div> {<img src={imageDefender} width={50} />}</div>
        <div>Attack:{props.selectedUnitDefenderData.attack}</div>
          <div>Defense: {props.selectedUnitDefenderData.defence}
            <span className={`hide-text ${props.selectedUnitDefenderData.defence_Bonus ? 'show-text' :  props.selectedUnitDefenderData.isCityWall ?  'show-text' :''}`}> (* {defenseBonus})</span>
            <span className={`hide-text ${props.selectedUnitDefenderData.isPoisoned ? 'show-text-red' :''}`}> ({props.selectedUnitDefenderData.default_Bonus})</span>
          </div>
        <div>Original HP {props.selectedUnitDefenderData.inputHealth} (max:{defenderMaxHPToUse}) <span style={{color: "green"}}>{props.selectedUnitDefenderData.isVeteran ?  ` +${veteranBuff}` : ''} </span> </div>
          <div>Status:
            <span className={`show-text ${defenceStatus.Title === "Defeated" ? 'show-text-red' : ''}`}>  {defenceStatus.Title}</span>
          </div>
          <div className={` small-image-size ${defenceStatus.Survived ? 'hide-image' : ''}`}>
            <img className="small-image-size" src="/misc-images/Vengir_Skull.png" />
          </div>
    </div>
